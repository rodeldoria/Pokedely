...
export function BattleModal({ wild: wildProp, trainerName, onClose }: Props) {
  ...
  const isTrainerBattle = !!trainerName;
  ...
  useEffect(() => {
    ...
    setPhase('menu');
    setLog(isTrainerBattle
      ? `${trainerName} sent out ${displayName(wildProp)}!`
      : `A wild ${displayName(wildProp)} appeared!`);
    setFeedback('');
  ...
  function safeExit(result: BattleResult) {
  ...
  function enemyTurn() {
    ...
    setTimeout(() => {
      ...
      if (newHp <= 0) {
        ...
        setPhase('result');
        setTimeout(() => safeExit({ caught: false, defeated: false }), 1800);
      } else {
        setTimeout(() => {
          setPhase('menu');
  ...
  function pickMove(m: Move) {
    ...
    setTimeout(() => {
      ...
      if (newHp <= 0) {
        if (isTrainerBattle) {
          setLog(`${trainerName}'s ${displayName(wild)} fainted!`);
          setFeedback(`🏆 You beat ${trainerName}!`);
          setPhase('result');
          Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(() => {});
          setTimeout(() => safeExit({ caught: false, defeated: true }), 1800);
        } else {
  ...
  function handleAnswer(idx: number) {
    ...
    if (correct) {
      ...
      if (next <= 0) {
        setTimeout(() => safeExit({ caught: false, defeated: false }), 1500);
      } else {
        setTimeout(() => {
  ...
  function throwBall(answeredRight: boolean) {
    ...
    setTimeout(() => {
      if (didCatch) {
        ...
        setPhase('result');
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(() => {});
        setTimeout(() => safeExit({ caught: true, defeated: false }), 2000);
      } else {
        const next = attemptsLeft - 1;
        setAttemptsLeft(next);
  ...
  function handleFlee() {
    if (isTrainerBattle) {
      setFeedback("You can't run from a trainer battle!");
      setTimeout(() => setFeedback(''), 1400);
      return;
    }
    setPhase('result');
    setFeedback(`${displayName(wild)} wandered off… maybe next time!`);
    setTimeout(() => safeExit({ caught: false, defeated: false }), 1000);
  }
    ...
    <Modal visible animationType="fade" transparent={false}>
      <View style={[styles.root, { backgroundColor: colors.background }]}>
        <View style={styles.arena}>
          ...
          <Animated.View
            style={[styles.wildPos, { transform: [{ translateX: shakeTx }] }]}
          >
          ...
          <View style={[styles.hpCard, styles.wildHpCard, { backgroundColor: colors.dialogue }]}>
            <View style={styles.hpHeader}>
              <Text style={[styles.monName, { color: colors.dialogueText }]} numberOfLines={1}>
                {isTrainerBattle ? `${trainerName}'s ${displayName(wild)}` : displayName(wild)}
              </Text>
          ...
          {phase === 'menu' && (
            <View style={styles.grid2x2}>
              ...
              <View style={styles.row}>
                ...
                <RetroButton
                  title="Run"
                  subtitle={isTrainerBattle ? 'Locked in' : 'Flee battle'}
...
const styles = StyleSheet.create({
  ...
  arena: {
    ...
    borderBottomWidth: 3, borderBottomColor: '#0a0a0a',
  },
  ...
  hpCard: {
    position: 'absolute', padding: 8, borderWidth: 2, borderColor: '#1d1f24',
  ...
  dialogue: {
    ...
    borderTopWidth: 3, borderBottomWidth: 3, borderColor: '#0a0a0a',
  },
